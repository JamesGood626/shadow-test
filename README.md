...
* [`src/cljs/shadow_test/`](src/cljs/shadow_test/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/shadow_test/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `shadow-test` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/shadow_test/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns shadow-test.example
  (:require [shadow-test.config :as config])
